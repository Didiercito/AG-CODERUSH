...
class EvaluadorFitness:
    ...
    def evaluar_individuo(self, individuo, problemas, participantes, config):
        try:
            ...
            puntuacion_norm = self._calcular_puntuacion_normalizada(asignaciones, problemas)
            compatibilidad_prom = self._calcular_compatibilidad_promedio(asignaciones)
            tiempo_norm = self._calcular_tiempo_normalizado(asignaciones, problemas)
            balance_norm = self._calcular_balance_participantes(asignaciones, len(participantes))
            ...
            logger.error(f"Dato faltante en los datos de entrada: {e}")
            individuo.fitness, individuo.es_valido = 0.0, False
        except Exception:
            individuo.fitness, individuo.es_valido = 0.0, False
        ...
        for i in range(matriz.shape[0]):
            for j in range(matriz.shape[1]):
                if matriz[i, j] > 0:
                    ...
                    puntos = self._obtener_puntos(problema)
                    prob_exito = self._calcular_probabilidad_exito(participante, problema)
                    asignaciones.append({'problema_idx': i, 'participante_idx': j, 'compatibilidad': self._calcular_compatibilidad(participante, problema), 'tiempo_estimado': self._estimar_tiempo(participante, problema), 'puntuacion_esperada': puntos * prob_exito})
    ...
    def _obtener_tasa_exito_participante(self, participante):
        return participante['tasa_exito_historica']

    def _calcular_compatibilidad(self, participante, problema):
        tasa_exito = self._obtener_tasa_exito_participante(participante)
        habilidades_p = participante['habilidades']
        habilidades_r = problema['habilidades_requeridas']
    ...
    def _calcular_probabilidad_exito(self, participante, problema):
        factores = {'muy_facil': 1.2, 'facil': 1.1, 'medio': 1.0, 'dificil': 0.9, 'muy_dificil': 0.8}
        factor = factores[problema['nivel_dificultad']]
        return self._validar_valor(self._calcular_compatibilidad(participante, problema) * factor)

    def _estimar_tiempo(self, participante, problema):
        return problema['tiempo_limite'] * (1.5 - self._calcular_compatibilidad(participante, problema))

    def _obtener_puntos(self, problema):
        return problema['puntos_base'] * problema['multiplicador_dificultad']
    
    def _calcular_puntuacion_normalizada(self, asignaciones, problemas):
        p_max = sum(self._obtener_puntos(p) for p in problemas)
    ...
    def _calcular_tiempo_normalizado(self, asignaciones, problemas):
        t_max = sum(p['tiempo_limite'] for p in problemas)
    ...
    def iniciar_optimizacion(self):
        ...
        for _ in range(self.generaciones_max):
            ...
            while len(nueva_poblacion) < self.poblacion_size:
                # <<< CAMBIO CLAVE: Llamar a la función dos veces, una para cada padre.
                padre1 = self.poda_por_seleccion(poblacion)
                padre2 = self.poda_por_seleccion(poblacion)
